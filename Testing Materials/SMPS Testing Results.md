...
For the 2nd image above, the load connected to the USB-C was the [DROK USB Load Tester](https://a.co/d/2gTHWYz). A [Power-Z KM003C](https://a.co/d/hrM3BIw) USB-C oscilloscope reader used to record the voltage, current, and power rating supplied from Channel-C. Unfortunately, there was no good way of saving images from this oscilloscope. The image above shows the results when the USB-C load tester draws 0.5A of current, displaying that the voltage does give a steady 5V. After this, the load tester was adjusted up to 3A, and the output power was about 12.6W. As the current went up, the voltage on the USB line slowly dropped until it roughly reached 4.2V, which is why the power for the load was 12.6W, not 15W. 

From here, the USB Load Tester was disconnected, and instead, a Samsung Galaxy Tab A SM-T350 was used as a load to verify USB-C charging would work on a common electronic device. The tablet uses a micro USB charging port, so a USB-C to micro USB adapter was used. After turning on the circuit, the tablet displayed it was successfully charging at 4.74V, 0.97A, for a total of 4.61W. This can be seen in the image below.
![USBC_Tablet_Charging](Images/USBC_Tablet_Charging.jpg)

Finally, a Google Pixel 9A was also used to verify charging capabilities, and the USB-C output still works. It was charging the device at  4.69V, 1.06A, for a total of 5.0W. The image for this can be seen below. Also, from the USB oscilloscope, it can be seen that the controller circuit was able to properly detect the different charging protocols and output the output as intended. The protocol for charging the tablet is Apple 2.4A, and the protocol for charging the phone was DCP 1.5A.
![USBC_Phone_Charging](Images/USBC_Phone_Charging.jpg)
